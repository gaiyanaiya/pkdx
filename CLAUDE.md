...
- **threat-screen** — `box/threats/` の環境ポケモンリストを参照し、指定ポケモンへの脅威（弱点技持ち・STAB耐性持ち）をスクリーニングして `box/teams/` に出力
...
.claude/skills/
  ...
  threat-screen/
    SKILL.md              # 脅威スクリーニングスキル（box/threats/ 参照 → box/teams/ 出力）
  self-update/
...
軸ポケモン名と `box/threats/` のファイルを受け取り、Phase 1でタイプ情報を取得・弱点を算出、Phase 2で各脅威ポケモンを3カテゴリ（STAB弱点技 / 非STAB弱点技 / STAB耐性）に分類、Phase 3で優先度ランク付けを行い、`box/teams/<ポケモン名>軸.md` として出力する。判定は threats ファイルに記載された技のみを対象とする。
